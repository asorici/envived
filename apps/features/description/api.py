...
class DescriptionAnnotationResource(AnnotationResource):
    
    class Meta(AnnotationResource.Meta):
        queryset = DescriptionAnnotation.objects.all()
        resource_name = DescriptionAnnotation.CATEGORY
    ...
    def build_filters(self, filters = None):
        ...
        orm_filters = super(DescriptionAnnotationResource, self).build_filters(filters)
        
        ## we now that there has to be a category in filters because of the validation
        categ_specific_filters = DescriptionAnnotation.get_extra_filters(filters)
    ...
    def hydrate(self, bundle):
        ...
        category = DescriptionAnnotation.CATEGORY
        ...
        try:
            bundle.obj = DescriptionAnnotation(user=user, environment=environment, area=area, category=category, data=data)
